...
const LINK_CLASS = "text-[13px] text-[var(--project-accent,#e4e0d4)] hover:opacity-80";
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8">
      <SectionHeader
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            <Link href={`/projects/new/intake?projectId=${params.projectId}`} className={LINK_CLASS}>
              Enrich intake
            </Link>
          </div>
        }
      ...
      {knowledge.avoidDoNotUse ? (
        <section className="mt-8 rounded-2xl border border-red-900/25 bg-red-950/10 p-4">
          <h3 className="text-[13px] font-medium text-white/60">Avoid / do not use</h3>
          <p className="mt-2 text-sm whitespace-pre-wrap text-white/80">{knowledge.avoidDoNotUse}</p>
        </section>
      ) : null}
      ...
      <div className="mt-10 space-y-1">
        ...
        >
          {strategy.positioning ? <p className="text-white/80">{strategy.positioning}</p> : null}

          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <h4 className="mb-2 text-[13px] text-white/45">Active phases</h4>
              {activePhases.length ? (
                <ul className="space-y-1.5">
                  {activePhases.map((p) => (
                    <li key={p.id} className="text-sm">{p.name} <span className="text-white/40">, {p.type}</span></li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm text-white/35">No active phases</p>
              )}
            </div>
            <div>
              <h4 className="mb-2 text-[13px] text-white/45">Recent decisions</h4>
              {recentDecisions.length ? (
                <ul className="space-y-2">
                  {recentDecisions.map((d) => (
                    <li key={d.id} className="border-l-2 pl-3 text-sm" style={{ borderColor: op?.color }}>
                      <p>{d.decision}</p>
                      {d.rationale ? <p className="text-[13px] text-white/45">{d.rationale}</p> : null}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm text-white/35">No decisions logged</p>
              )}
            </div>
          </div>

          <Link href={`/projects/${params.projectId}/strategy`} className={`inline-block ${LINK_CLASS}`}>
            Open strategy doc →
          </Link>
        </CollapsibleSection>
        ...
        >
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <h4 className="mb-2 text-[13px] text-white/45">Important sources</h4>
              <ul className="space-y-1.5">
                {keySources.map((s) => (
                  <li key={s.id}>
                    <Link href={`/projects/${params.projectId}/sources/${s.id}`} className="text-sm text-[var(--project-accent,#e4e0d4)]">
                      {s.title}
                    </Link>
                    {s.isSourceOfTruth ? <span className="ml-2 text-[12px] text-white/40">Source of truth</span> : null}
                  </li>
                ))}
                {!keySources.length ? <p className="text-sm text-white/35">No sources yet</p> : null}
              </ul>
            </div>
            <div>
              <h4 className="mb-2 text-[13px] text-white/45">Key references</h4>
              <ul className="space-y-1.5">
                {keyRefs.map((r) => (
                  <li key={r.id} className="text-sm">{r.title}</li>
                ))}
                {!keyRefs.length ? <p className="text-sm text-white/35">No references yet</p> : null}
              </ul>
            </div>
          </div>
          ...
          {knowledge.openQuestions ? (
            <div>
              <h4 className="mb-1 text-[13px] text-white/45">Open questions</h4>
              <p className="text-sm whitespace-pre-wrap text-white/70">{knowledge.openQuestions}</p>
            </div>
          ) : null}
          {knowledge.terminology ? (
            <div>
              <h4 className="mb-1 text-[13px] text-white/45">Terminology</h4>
              <p className="text-sm whitespace-pre-wrap text-white/70">{knowledge.terminology}</p>
            </div>
          ) : null}

          <div className="flex gap-4">
            <Link href={`/projects/${params.projectId}/sources`} className={LINK_CLASS}>
              Manage sources →
            </Link>
            <Link href={`/projects/${params.projectId}/links`} className={LINK_CLASS}>
              Manage links →
            </Link>
            <Link href={`/projects/${params.projectId}/references`} className={LINK_CLASS}>
              Manage references →
            </Link>
          </div>
        </CollapsibleSection>
        ...
        >
          <div>
            <h4 className="mb-2 text-[13px] text-white/45">Upcoming deliverables</h4>
            {upcomingDeliverables.length ? (
              <ul className="space-y-1.5">
                {upcomingDeliverables.map((d) => (
                  <li key={d.id} className="flex items-center gap-2 text-sm">{d.name} <StatusPill>{d.status.replace("_", " ")}</StatusPill></li>
                ))}
              </ul>
            ) : (
          ...
          <div className="flex gap-4">
            <Link href={`/projects/${params.projectId}/deliverables`} className={LINK_CLASS}>
              Manage deliverables →
            </Link>
            <Link href={`/projects/${params.projectId}/content`} className={LINK_CLASS}>
              Manage content plan →
            </Link>
          </div>
        </CollapsibleSection>
...
function Brief({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-1 sm:grid-cols-[140px_1fr] sm:gap-4">
      <h3 className="text-[13px] text-white/45">{label}</h3>
