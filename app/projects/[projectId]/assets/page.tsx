  ...
  return (
    <main className="px-6 py-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-[-0.01em]">Assets</h1>
          <p className="mt-2 text-sm text-white/55">
            {cloudEnabled ? "Stored in Supabase Storage when uploaded." : "Local cache only — configure Supabase for cloud storage."}
          </p>
        </div>
        <label className="h-10 cursor-pointer rounded-xl border border-white/20 px-4 text-[13px] leading-10">
          Upload
          <input
            type="file"
            accept="image/*,.svg"
        ...
        {uploadStatus ? <span className="text-xs text-white/50">{uploadStatus}</span> : null}
      </div>
      {CATEGORIES.map((category) => {
        ...
        return (
          <section key={category} className="mt-10">
            <h2 className="mb-4 text-[15px] font-medium text-white/60 capitalize">{category}</h2>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {items.map((asset) => (
                <div key={asset.id} className="rounded-2xl border border-white/10 p-2">
                  ...
                  <input
                    ...
                    className="mt-2 w-full bg-transparent text-[13px]"
                  />
                  <button
                    type="button"
